...
impl IntervalRef {
    ...
    /// update this cache based on the result of this method.
    pub fn find(&mut self, position: isize) -> Self {
        ...
        loop {
            ...
            if relative_position < tree.left_total_length() {
                tree = tree.get_left_raw();
            } else if let Some(right) = tree.get_right() {
                if relative_position >= tree.total_length - right.total_length {
                    relative_position -= tree.total_length - right.total_length;
                    tree = right;
                }
            } else {
                tree.position =
    ...
    pub fn next(self) -> Self {
        ...
        let next_position = interval.position + interval.length();
        if let Some(right) = interval.get_right() {
            interval = right;
            while let Some(left) = self.get_left() {
                interval = left;
            }
            interval.position = next_position;
            return interval;
        }
        unimplemented!()
    }
...
/*#[no_mangle]
pub extern "C" fn find_interval(mut tree: IntervalRef, position: isize) -> IntervalRef {
    tree.find(position)
}*/
